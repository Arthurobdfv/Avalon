# Avalon

## Overview
Avalon is an MMO prototype project created to validate game design ideas and practice Unity development and game server implementation.

## Purpose
This project serves as:
- A proof of concept for MMO mechanics and architecture
- A learning platform for Unity game development
- An opportunity to implement and test game server technologies

## Status
🚧 Prototype/Work in Progress

Project progress & feature tracker: `FEATURES/FEATURES.md`

## Project Progress

Check the `FEATURES/FEATURES.md` for full details. Quick summary:

- **Features:**
  - [SpriteSheet to Animations Pipeline](FEATURES/SpriteSheet-to-Animations.md) — in progress; slicing implemented and animator-transition prototype added
  - [Basic Entity Manager & Combat Flow](docs/feature-basic-entity-manager-and-combat.md) — implemented (basic) (implementation notes: `docs/CombatSystem.md`)

- **Documentation:**
  - Changelog: `CHANGELOG.md`
  - Feature index: `FEATURES/FEATURES.md`

- **How to contribute:** update `FEATURES/FEATURES.md` with feature status or add a new detailed feature doc under `FEATURES/` or `docs/` and link it here. Update `CHANGELOG.md` under `Unreleased` when making notable changes.

## Credits
### Assets
Game assets purchased from [PVGames (Pioneer Valley Games)](http://www.pioneervalleygames.com/)

## AI Usage Disclaimer
Some portions of this README and other documentation were generated or assisted by AI tools to enhance clarity and organization.


## License
This is a prototype project for educational and validation purposes.

